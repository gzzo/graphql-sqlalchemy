...
def make_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    args = {
...
def make_pk_args(model: type[DeclarativeBase]) -> GraphQLArgumentMap:
...
def make_insert_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    return {
...
def make_insert_one_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    return {
...
def make_delete_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    return {"where": GraphQLArgument(get_where_input_type(model, inputs))}


def make_update_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    return {
...
def make_update_by_pk_args(model: type[DeclarativeBase], inputs: Inputs) -> GraphQLArgumentMap:
    return {
        **({"_inc": GraphQLArgument(get_inc_input_type(model, inputs))} if has_int(model) else {}),
        "_set": GraphQLArgument(get_set_input_type(model, inputs)),
        **make_pk_args(model),
